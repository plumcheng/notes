...
1物理层：利用传输介质为数据链路层提供物理连接，实现比特流的透明传输。这一层的数据叫做比特。

2数据链路层：接收来自物理层的位流形式的数据，并封装成帧，传送到上一层 
...
因为这个ACK有可能丢失，从而导致处在 LAST-ACK 状态的服务器收不到对 FIN-ACK 的确认报文。服务器会超时重传这个FIN-ACK，接着客户端再重传一次确认，重新启动时间等待计时器。最后客户端和服务器都能正常的关闭。假设客户端不等待2MSL，而是在发送完ACK之后直接释放连接，一但这个ACK丢失的话，服务器就无法正常的进入关闭连接状态。

2、防止 “已失效的连接请求报文段”  出现在本连接中。

经过2MSL，本连接持续时间内所产生的所有报文段都从网络中消失，使下一个新的连接不会出现这种旧的连接请求报文段。

##### 拥塞控制：

拥塞窗口（cwnd） 慢开始门限（ssthresh） 慢开始算法、拥塞避免算法、快重传、快恢复。

拥塞窗口的大小取决于网络的拥塞程度，动态的变化，发送方让自己的窗口等于拥塞窗口。

只要出现超时就认为网络出现了拥塞。根据发送方的最大报文段SMSS的数值设置拥塞窗口。 

先设置慢开始门限，

当 cwnd < ssthresh 使用慢开始算法; 当 cwnd > ssthresh 使用拥塞避免算法

慢开始算法：每收到一个确认报文，就将拥塞窗口值加一； 拥塞避免算法：每次经过一个传输轮次，就将拥塞窗口值加一（一个传输轮次经历的时间就是往返时间RTT，RTT随拥塞窗口值的大小改变）；

当网络出现超时时，调整慢开始门限 ssthresh = cwnd / 2 （cwnd为网络出现超时时的值） 同时设置cwnd = 1；重新进入慢开始阶段。

快重传：如果发送方收到连续三个对同一个报文段的重复确认。

快恢复：



ps: 拥塞窗口大小的单位实际是 字节数，一般简单将报文段的个数作为窗口大小的单位
